...
.bss
		.align 2
@; matrices de recombinaciï¿½n: matrices de soporte para generar una nueva matriz
...
@;	juego, primero cargando el mapa de configuraciï¿½n indicado por parï¿½metro (a
@;	obtener de la variable global 'mapas'), y despuï¿½s cargando las posiciones
@;	libres (valor 0) o las posiciones de gelatina (valores 8 o 16) con valores
@;	aleatorios entre 1 y 6 (+8 o +16, para gelatinas)
@;	Restricciones:
@;		* para obtener elementos de forma aleatoria se invocarï¿½ la rutina
@;			'mod_random'
@;		* para evitar generar secuencias se invocarï¿½ la rutina
...
@;	mediante la reubicaciï¿½n de los elementos de la matriz original, para crear
@;	nuevas jugadas.
@;	Inicialmente se copiarï¿½ la matriz original en 'mat_recomb1', para luego ir
...
@;			invocarï¿½ la rutina 'hay_combinacion' (ver fichero "candy1_comb.s")
@;		* se supondrï¿½ que siempre existirï¿½ una recombinaciï¿½n sin secuencias y
@;			con combinaciones
@;	Parámetros:
@;		R0 = dirección base de la matriz de juego
...
@;	Restricciones:
@;		* el parï¿½metro 'n' tiene que ser un valor entre 2 y 255, de otro modo,
@;		  la rutina lo ajustarï¿½ automï¿½ticamente a estos valores mï¿½nimo y mï¿½ximo
@;	Parï¿½metros:
@;		R0 = el rango del nï¿½mero aleatorio (n)
@;	Resultado:
...
mod_random:
		...
		cmp r0, #2				@;compara el rango de entrada con el mï¿½nimo
		bge .Lmodran_cont
		mov r0, #2				@;si menor, fija el rango mï¿½nimo
	.Lmodran_cont:
		and r0, #0xff			@;filtra los 8 bits de menos peso
		sub r2, r0, #1			@;R2 = R0-1 (nï¿½mero mï¿½s alto permitido)
		mov r3, #1				@;R3 = mï¿½scara de bits
	.Lmodran_forbits:
		cmp r3, r2				@;genera una mï¿½scara superior al rango requerido
		bhs .Lmodran_loop
		mov r3, r3, lsl #1
	...
	.Lmodran_loop:
		bl random				@;R0 = nï¿½mero aleatorio de 32 bits
		and r4, r0, r3			@;filtra los bits de menos peso segï¿½n mï¿½scara
		cmp r4, r2				@;si resultado superior al permitido,
		bhi .Lmodran_loop		@; repite el proceso
		mov r0, r4			@; R0 devuelve nï¿½mero aleatorio restringido a rango
...
@; random(): rutina para obtener un nï¿½mero aleatorio de 32 bits, a partir de
@;	otro valor aleatorio almacenado en la variable global 'seed32' (declarada
@;	externamente)
@;	Restricciones:
@;		* el valor anterior de 'seed32' no puede ser 0
@;	Resultado:
@;		R0 = el nuevo valor aleatorio (tambiï¿½n se almacena en 'seed32')
random:
	push {r1-r5, lr}
		
	ldr r0, =seed32				@;R0 = direcciï¿½n de la variable 'seed32'
