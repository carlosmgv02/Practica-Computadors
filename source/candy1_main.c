/*------------------------------------------------------------------------------
	...
	(2潞 curso de Grado de Ingenier铆a Inform谩tica - ETSE - URV)
...
int seed32;						// semilla de n鷐eros aleatorios
...
/* candy1_main.c : funci髇 principal main() para test de tarea 1A 	*/
/*					(requiere tener implementada la tarea 1E)		*/
/* ---------------------------------------------------------------- */
int main(void)
{

/* candy1_main.c : funci髇 principal main() para test de tarea 1E 	*/
/* ---------------------------------------------------------------- */
...
int main(void)
{
	...
	consoleDemoInit();			// inicializaci髇 de pantalla de texto
...
/* candy1_main.c : funci贸n principal main() para test de tarea 1G 	*/
/*					(requiere tener implementada la tarea 1E)		*/
/* ---------------------------------------------------------------- */
...
int main(void)
{
	...
	do							// bucle principal de pruebas
	{
		...
		do
		{	swiWaitForVBlank();
			scanKeys();					// esperar pulsaci髇 tecla 'A' o 'B'
		...
		do
		{	swiWaitForVBlank();
			scanKeys();					// esperar pulsaci贸n tecla 'A' o 'B'
		} while (!(keysHeld() & (KEY_A | KEY_B)));
		printf("\x1b[3;0H                               ");
		retardo(5);
	...
	do							// bucle principal de pruebas
	{
		...
		if (keysHeld() & KEY_A)		// si pulsa 'A',
		{
			...
			{				// si n鷐ero de mapa del siguiente test diferente
				level = nmap[ntest];		// del n鷐ero de mapa actual,
